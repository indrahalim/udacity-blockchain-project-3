    ...
    RetailerRole
{
    ...
    modifier verifyCaller(address _address, address sender) {
        require(sender == _address);
        _;
    }

    // Define a modifier that checks if the paid amount is sufficient to cover the price
    modifier paidEnough(uint256 _price, uint256 paymentAmount) {
        require(paymentAmount >= _price);
        _;
    }

    // Define a modifier that checks the price and refunds the remaining balance
    modifier checkValue(uint256 _upc, uint256 paidAmount) {
        _;
    ...
    function buyItem(uint256 _upc)
        ...
        paidEnough(items[_upc].productPrice, msg.value)
        // Call modifer to send any excess ether back to buyer
        checkValue(_upc, msg.value)
    {
